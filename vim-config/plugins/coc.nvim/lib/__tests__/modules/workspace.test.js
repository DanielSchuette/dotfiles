...
describe('workspace properties', () => {
    it('should have initialized', () => {
        let { nvim, channelNames, rootPath, cwd, documents, initialized, textDocuments } = workspace_1.default;
...
describe('workspace applyEdits', () => {
    ...
    it('should merge TextEdits of same document', async () => {
        let doc = await helper_1.default.createDocument();
        await doc.applyEdits([vscode_languageserver_types_1.TextEdit.insert({ line: 0, character: 0 }, 'x = x')]);
        let versioned = vscode_languageserver_types_1.VersionedTextDocumentIdentifier.create(doc.uri, doc.version);
        let workspaceEdit = {
            documentChanges: [
                vscode_languageserver_types_1.TextDocumentEdit.create(versioned, [{ range: vscode_languageserver_types_1.Range.create(0, 0, 0, 1), newText: 'foo' }]),
                vscode_languageserver_types_1.TextDocumentEdit.create(versioned, [{ range: vscode_languageserver_types_1.Range.create(0, 4, 0, 5), newText: 'foo' }]),
            ]
        };
        let res = await workspace_1.default.applyEdit(workspaceEdit);
        expect(res).toBe(true);
    ...
    it('should support changes with edit and rename', async () => {
        let file = await helper_1.createTmpFile('test');
        let doc = await helper_1.default.createDocument(file);
        let newFile = path_1.default.join(os_1.default.tmpdir(), `coc-${process.pid}/new-${uuid_1.v4()}`);
...
describe('workspace methods', () => {
    it('should selected range', async () => {
        ...
        let range = await workspace_1.default.getSelectedRange('v', doc);
    ...
    it('should expand variables', async () => {
        ...
        let folder = path_1.default.dirname(workspace_1.default.root);
    ...
    it('should open temimal buffer', async () => {
        let bufnr = await workspace_1.default.openTerminal('ls', { autoclose: false, keepfocus: false });
    ...
    it('should rename buffer', async () => {
        await helper_1.default.createDocument('a');
...
describe('workspace utility', () => {
    ...
    it('should loadFiles', async () => {
        ...
        await workspace_1.default.loadFiles(files);
    ...
    it('should create file if parent folder not exists', async () => {
        const folder = path_1.default.join(__dirname, 'foo');
        const filepath = path_1.default.join(folder, 'bar');
    ...
    it('should overwrite if file exists', async () => {
        let filepath = path_1.default.join(os_1.default.tmpdir(), uuid_1.v4());
        let newPath = path_1.default.join(os_1.default.tmpdir(), uuid_1.v4());
        await workspace_1.default.createFile(filepath);
