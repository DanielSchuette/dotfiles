...
declare class Languages {
    ...
    getHover(document: TextDocument, position: Position, token: CancellationToken): Promise<Hover[]>;
    getSignatureHelp(document: TextDocument, position: Position, token: CancellationToken): Promise<SignatureHelp>;
    getDefinition(document: TextDocument, position: Position, token: CancellationToken): Promise<Location[]>;
    getDeclaration(document: TextDocument, position: Position, token: CancellationToken): Promise<Location[] | Location | LocationLink[] | null>;
    getTypeDefinition(document: TextDocument, position: Position, token: CancellationToken): Promise<Location[]>;
    getImplementation(document: TextDocument, position: Position, token: CancellationToken): Promise<Location[]>;
    getReferences(document: TextDocument, context: ReferenceContext, position: Position, token: CancellationToken): Promise<Location[]>;
    getDocumentSymbol(document: TextDocument, token: CancellationToken): Promise<SymbolInformation[] | DocumentSymbol[]>;
    getSelectionRanges(document: TextDocument, positions: Position[], token: any): Promise<SelectionRange[] | null>;
    getWorkspaceSymbols(query: string, token: CancellationToken): Promise<SymbolInformation[]>;
    ...
    getCodeActions(document: TextDocument, range: Range, context: CodeActionContext, token: CancellationToken): Promise<Map<string, CodeAction[]>>;
    getDocumentHighLight(document: TextDocument, position: Position, token: CancellationToken): Promise<DocumentHighlight[]>;
    getDocumentLinks(document: TextDocument, token: CancellationToken): Promise<DocumentLink[]>;
    ...
    provideColorPresentations(color: ColorInformation, document: TextDocument, token: CancellationToken): Promise<ColorPresentation[]>;
    getCodeLens(document: TextDocument, token: CancellationToken): Promise<CodeLens[]>;
    resolveCodeLens(codeLens: CodeLens): Promise<CodeLens>;
    provideDocumentOnTypeEdits(character: string, document: TextDocument, position: Position, token: CancellationToken): Promise<TextEdit[] | null>;
